...
消费者需要严格按照`Claim->处理->Free->下一次Claim`的顺序交替进行，不允许在上一个Claim还没有Free时，调用Claim申请新的数据包。
...
MPSC_PBUF/
├── Inc/
│   ├── mpsc_pbuf_internal.h  内部定义（valid / busy 标志位 + 跳过包结构）
...
> 注意：位序依赖编译器将第一个位域分配在最低有效位（LSB-first，主流小端目标均如此）；大端目标上 `valid` / `busy` 的位置会颠倒，本实现不适用。
...
MpscPbufInit(&buf, true, GetMsgWlen, g_buf, BUF_WORDS, MutexTake, MutexGive);
```
...
> 注意：`MpscPbufClaim` 与 `MpscPbufFree` 必须严格交替调用——上一个声明的包未释放前，不允许再次声明新的包。
...
static uint32_t GetMsgWlen(const MPSC_PBUF_GENERIC_T * pPacket)
{
    (void)pPacket;
...
int main(void)
{
    ...
    // 覆盖模式；无互斥锁 = 单线程/裸机场景
    MpscPbufInit(&pbuf, true, GetMsgWlen, g_buf, BUF_WORDS, NULL, NULL);

    // 生产：Alloc → 填充 → Commit（Commit 内部会设置 valid 位）
    msg_t * m = (msg_t *)MpscPbufAlloc(&pbuf, sizeof(msg_t) / sizeof(uint32_t));
    if (m) {
        m->hdr.id  = 42;
        m->hdr.len = 8;
